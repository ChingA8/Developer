...
wp_deregister_script( 'gjm-autocomplete' );
wp_register_script( 'gjm-autocomplete', get_template_directory_uri() . '/js/autocomp.min.js' );
...
if(is_page('2181')){
	//wp_enqueue_script( 'auto_job', get_template_directory_uri(). '/js/autocomp_job.js');
}
    ...
    if(is_user_logged_in()){
        ...
        if($user_meta):
                ...
                wp_enqueue_script( 'ajax_activation', get_template_directory_uri(). '/js/ajax_activation.js');
...
function my_login_redirect( $redirect_to, $request, $user ) {
	//is there a user to check?
		...
		if($redirect_to != 'http://scoot.my/login/' || $redirect_to != 'http://scoot.my/login/?login=failed')
		{
...
function scoot_registration_redirect() {
    $redirect_to = $_SERVER['HTTP_REFERER'];
    return $redirect_to;
}

add_filter( 'registration_redirect', 'scoot_registration_redirect' );
add_filter( 'jobify_registeration_redirect', 'scoot_registration_redirect' );
...
// Add field to admin
add_filter( 'resume_manager_resume_fields', 'wpjms_admin_resume_form_fields', 11 );
function wpjms_admin_resume_form_fields( $fields ) {

    $fields['_candidate_language'] = array(
        'label' 		=> __( 'Native Language', 'job_manager' ),
        'type' 			=> 'text',
        'placeholder' 	=> __( 'English', 'job_manager' ),
        'description'	=> '',
    );
...
add_filter( 'submit_resume_form_fields', 'wpjms_frontend_resume_form_fields' , 11);
function wpjms_frontend_resume_form_fields( $fields ) {

    $fields['resume_fields']['candidate_language'] = array(
        'label' => __( 'Native Language', 'job_manager' ),
        'type' => 'text',
        'required' => true,
        'placeholder' => 'English',
...
function wpjms_language_field_email_message( $message, $resume_id ) {
    $message[] = "\n" . "Native Language: " . get_post_meta( $resume_id, '_candidate_language', true );
    return $message;
}
*/